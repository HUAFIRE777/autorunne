# Autorunne 自动识别 / 自动初始化 / 自动恢复（0.6.17）
...
## 现在的落地方式
### 1. `autorunne open`
`autorunne open` 依然是进入仓库的核心入口；到 0.6.17，状态层、视图层、direct agent 入口、版本升级提示、多包项目扫描、轻量 Python 项目识别和全 agent repo 入口已经形成一条可演示的主路径。旧 workspace 可迁移，status 读 state，task 可显式操作，恢复提示会自动忽略 `.vscode/` 这类噪音。
...
## 当前阶段最重要的事
到 0.6.17，Autorunne 已经把“状态层 / 视图层 / skill 接入层 / wrapper 接入层 / legacy migration / 显式 task 操作 / direct agent / 多包项目扫描 / 轻量 Python 识别 / Cursor 和 Copilot 入口”打通成一条主路径。
