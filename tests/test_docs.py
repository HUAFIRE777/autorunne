...
def test_chinese_manual_documents_monorepo_support():
    ...
    assert "autorunne release --version 0.6.20" in text
...
def test_usage_guides_are_updated_for_0613_monorepos():
    ...
    for text in (zh, en):
        ...
        assert "autorunne-0.6.20-py3-none-any.whl" in text
        assert "autorunne release --version 0.6.20" in text
...
def test_business_docs_position_autorunne_as_repo_local_memory_layer():
    ...
    assert "0.6.20 更适合真实项目演示" in sales

    assert "Autorunne 0.6.20 发布说明" in release
    assert "PyPI：`autorunne==0.6.20`" in release
